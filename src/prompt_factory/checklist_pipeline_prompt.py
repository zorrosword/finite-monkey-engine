class ChecklistPipelinePrompt:
    ...
    def generate_project_type_checklist_prompt(language,project_type_list):
        ...
        3. 将所有检查项汇总后，按照项目类型分类和整理，形成清晰明确的不同项目下的检查清单。
        4. 确保检查清单完整覆盖项目中所有可能发生漏洞的技术栈、协议、流程，不遗漏任何关键环节。
        5. 必须直接输出完整版的清单，不能有任何如"这些只是一些主要类型的检查项，完整的清单会更详细"之类的描述
        </instructions>

        <output_format>
        以清晰详细的列表形式输出，每个检查项需明确项目类型漏洞标题、漏洞描述、检查内容，格式示例如下：
    ...
    def merge_project_type_checklist(language, project_type_lists):
        ...
        5. 合并需要以*项目类型*为主线，保持原有的逻辑结构，且需要保持描述的准确性和完整性
        6. 重新组织和优化检查项的表述，使其更加清晰和系统
        7. 必须直接输出完整版的清单，不能有任何如"这些只是一些主要类型的检查项，完整的清单会更详细"之类的描述
        </instructions>
